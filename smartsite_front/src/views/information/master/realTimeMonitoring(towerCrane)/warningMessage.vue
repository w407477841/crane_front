...
export default {
  ...
  mounted() {
    ...
    this.isOnline = this.$route.query.row.status==1?"在线":"离线";
  ...
  methods: {
    ...
    goToMessageAlarm : function(row) {
      this.id = row.id;
      this.content = "工程名称：" + this.mainInfo.projectName + ",设备编号：" + this.mainInfo.deviceNo + ",总报警：" + row.alarmAll + ",重量报警：" + row.alarmWeight + ",幅度报警：" + row.alarmRange + ",力矩报警 ：" + row.alarmMoment + ",碰撞报警：" + row.alarmCollision + ",风速报警：" + row.alarmSpeed + ",倾斜报警：" + row.alarmTiltAngle;
