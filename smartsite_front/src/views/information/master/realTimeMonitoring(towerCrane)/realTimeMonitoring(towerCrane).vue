    ...
    <div class="container-box list-box">
      <div class="table-box">
        <el-table size="small" :data="listData" style="width: 100%" height="100%" highlight-current-row @selection-change="listSelectionChange" header-row-class-name="xyTableHeader">
          ...
          <el-table-column label="操作" align="center" width="700" fixed="right">
            <template slot-scope="scope">
              ...
              <el-button type="primary" plain size="small" @click="goToPromptingMessage(scope.row)">预警信息</el-button>
...
export default {
  ...
  methods: {
    ...
    goToRunTime : function(row) {
      this.$router.push({ path: this.runTimePath, query: { row : row } });
    },
    //点击吊重数据按钮跳转到吊重数据页面
    goToHoistingData : function(row) {
      this.$router.push({ path: this.hoistingDataPath, query: { row : row } });
    },
    //点击预警信息按钮跳转到预警信息页面
