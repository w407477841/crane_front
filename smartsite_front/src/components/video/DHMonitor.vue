...
<template>
    <div class="monitor">
      <div class="tajialert">
					...
					<div id="f_ocx" style="top:5rem;left:0;width:72%;height:72%;"></div>
			</div>
    </div>
</template>
...
export default {
  ...
  watch: {
    listMaterialMonitor() {
      this.openDahua(this.flag);
    }
  },
  methods: {
    openDahua(flag) {
      var that = this;
      console.log("35", that.listMaterialMonitor);
      var info = {
        ip: that.listMaterialMonitor[flag].ip,
        port: that.listMaterialMonitor[flag].port,
        username: that.listMaterialMonitor[flag].username,
        password: that.listMaterialMonitor[flag].password
      };
      var mainOcxHtml = "";
