...
<template>
  <div class="uploadFile">
    <div>
      <el-button @click="uploadClick" size="mini" type="primary" :disabled="disabled || fileList.length==limit">选择文件</el-button>
      <span class="file_solt_upload_name" :style="style">{{fileName}}</span>
      <input :id="DomId" @change="inputChange($event)" ref="upload" type="file" class="file_solt_upload_input" size="mini" value="选择文件" :accept="accept" />
      <el-button size="mini" icon="el-icon-upload" type="primary" :disabled="disabled" @click="submitUpload">文件上传</el-button>
      <el-button type="danger" icon="el-icon-delete" size="mini" :disabled="disabled || selsdetails.length == 0" @click="deleteRowDetails">删除</el-button>
    </div>
    <el-table ref="multipleTable" size="small" :data="datas" style="width: 100%" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" width="150" v-if="newHide" >
        <template slot-scope="scope">
          <a style="color:#0698d6;" :href="[fileBaseUrl+'/fileUpload/filedownload?remotePath='+scope.row.path]">点击下载</a>
        </template>
      </el-table-column>
    </el-table>
  </div>
