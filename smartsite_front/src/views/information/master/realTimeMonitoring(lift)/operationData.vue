    ...
    <div class="container-box list-box">
      <div class="table-box">
        <el-table size="small" :data="listData" style="width: 100%" height="100%" highlight-current-row @selection-change="listSelectionChange" header-row-class-name="xyTableHeader">
          ...
          <el-table-column label="运行时间" prop="deviceTime"></el-table-column>
...
export default {
  data() {
    return {
      ...
      beginDate: "",
      endDate: "",
      statusList: [{ label: "受控", value: 1 }, { label: "不受控", value: 0 }],
      momentPercentage: "",
  ...
  mounted() {
    console.log("缓存数据",sessionStorage.getItem("liftDeviceNo"))
   console.log(this.$route.query)
       this.deviceNo = this.$route.query.row.deviceNo==null?sessionStorage.getItem("liftDeviceNo"):this.$route.query.row.deviceNo;
    console.log("升降机", this.$route.query.row.isOnline);
    if(this.$route.query.row.isOnline==null || this.$route.query.row.isOnline==""){
      this.isOnline = sessionStorage.getItem("liftIsOnline");
    }else{
      this.isOnline =this.$route.query.row.isOnline;
    }
    console.log("升降机", this.isOnline);
    
    this.projectName = this.$route.query.row.projectName==null?sessionStorage.getItem("liftProjectName"):this.$route.query.row.projectName;
    this.liftId = this.$route.query.row.id;
